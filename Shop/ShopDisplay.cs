...
public class ShopDisplay : MonoBehaviour
{
    ...
    int shopNumber; // itemInInven의 [i] 번째를 저장하는 변수. //현재 선택된 아이템의 번호. >>0부터 시작
    ...
    public void SelectingItem()
    {
        ...
        for (int i = 0; i < shopTotal; i++)
        {
            if (i == shopNumber) //선택된 아이템의 정보 출력
            {
                ...
                infoText.selectedItem = inventory.Container[itemInShop[i].GetComponent<IsGone>().itemID].item; //현재 선택된 아이템의 정보를 infoText에 전달.
            }
            else//선택되지 않은 아이템들
            {
    ...
    public void buyItem() //itemuseUI에서 사용버튼을 누를경우 실행되는 함수.
    {
