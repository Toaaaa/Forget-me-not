...
public class MagiSkill4 : PlayerSkill//피어싱 라이트닝
{
    public void targetLocked()
    {
        ...
        if (targetMob != null)
            transform.DOMove(targetMob.GetComponent<TestMob>().bottomPos.transform.position, 1.6f).SetEase(Ease.Linear);//투사체1.6초 동안 이동
        else
        {
            Destroy(gameObject);
        }
    ...
    private async void OnTriggerEnter2D(Collider2D collision)
    {
        if (collision.gameObject == targetMob.gameObject)
        {
            ...
            await UniTask.Delay(1800);//천둥 애니메이션 재생후 데미지 모션과 적용의 동기화를 위한 딜레이.
